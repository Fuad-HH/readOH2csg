...
def get_edge_coefficients(mesh: OmegaHMesh, print_debug=False, tol=1e-6):
    ...
    try:
        _dll.capi_compute_edge_coefficients(
            mesh.mesh, c_int(size), coefficients, print_debug, tol
        )
...
def get_face_connectivity(
    mesh: OmegaHMesh, edge_coefficients=None, print_debug=False, tol=1e-6
):
    if not kokkos_runtime.is_running():
        raise RuntimeError("Kokkos not running...")
...
def create_openmc_geometry(
    ...
    [edge_coefficients, boundary_edge_ids, face_connctivity] = get_all_geometry_info(
        mesh, print_debug, tol
    )
    n_edges = mesh.num_entities(1)
    n_faces = mesh.num_entities(2)
