...
dg2d_converted_mesh = parent_dir / 'assets/simple_degas2_case/definegeometry2d-geometry.nc'
...
def test_get_integer_tag_array():
    with OmegaHMesh(omega_h_mesh_to_convert) as mesh:
        ...
        assert num_offset_faces == 28, ("For this mesh, there are 28 offset faces."
...
def test_get_cell_volumes():
    ...
    with nc.Dataset(gold_nc_mesh, "r", format="NETCDF4_CLASSIC") as mesh:
        ...
        assert np.isclose(oh_total_volume, gold_nc_total_volume,
                          rtol=1e-5), (f"Total volume mismatch. "
                                       f"{oh_total_volume=} , {gold_nc_total_volume=}")
    ...
    with nc.Dataset(dg2d_converted_mesh, "r", format="NETCDF4_CLASSIC") as mesh:
        ...
        assert np.isclose(oh_total_volume, dg2d_total_volume, rtol=1e-5), (f"Total volume mismatch. "
                                                                           f"{oh_total_volume=} , {dg2d_total_volume=}")

    for i in range(0, num_inside_faces):
        assert np.isclose(gold_nc_zone_volumes[i], oh_cell_volumes[i], rtol = 1e-5), f"{gold_nc_zone_volumes[i]} ≠ {oh_cell_volumes[i]} for face {i}"
        assert np.isclose(dg2d_nc_zone_volumes[i], oh_cell_volumes[i], rtol = 1e-5), f"{dg2d_nc_zone_volumes[i]} ≠ {oh_cell_volumes[i]} for face {i}"
...
def test_get_cell_bounding_boxes():
    ...
    for i in range(0, num_inside_faces):
        # xmin
        coord_tol = 1e-5
        assert np.isclose(oh_cell_bboxes[i*4+0], gold_nc_bbox_mins[i,0], atol=coord_tol), f"Bounding box mismatch for face {i}"
        assert np.isclose(oh_cell_bboxes[i*4+0], dg2d_nc_bbox_mins[i,0], atol=coord_tol), f"Bounding box mismatch for face {i}"
        # ymin
        assert np.isclose(oh_cell_bboxes[i*4+1], gold_nc_bbox_mins[i,2], atol=coord_tol), f"Bounding box mismatch for face {i}"
        assert np.isclose(oh_cell_bboxes[i*4+1], dg2d_nc_bbox_mins[i,2], atol=coord_tol), f"Bounding box mismatch for face {i}"
        # xmax
        assert np.isclose(oh_cell_bboxes[i*4+2], gold_nc_bbox_maxs[i,0], atol=coord_tol), f"Bounding box mismatch for face {i}"
        assert np.isclose(oh_cell_bboxes[i*4+2], dg2d_nc_bbox_maxs[i,0], atol=coord_tol), f"Bounding box mismatch for face {i}"
        # ymax
        assert np.isclose(oh_cell_bboxes[i*4+3], gold_nc_bbox_maxs[i,2], atol=coord_tol), f"Bounding box mismatch for face {i}"
        assert np.isclose(oh_cell_bboxes[i*4+3], dg2d_nc_bbox_maxs[i,2], atol=coord_tol), f"Bounding box mismatch for face {i}"
...
def test_get_cell_centroids():
    ...
    for i in range(0, num_inside_faces):
        coord_tol = 1e-5
        assert np.isclose(oh_cell_centroids[i*2+0], gold_zone_centroids[i, 0], atol=coord_tol), f"Centroid x mismatch for face {i}"
        assert np.isclose(oh_cell_centroids[i*2+0], dg2d_zone_centroids[i, 0], atol=coord_tol), f"Centroid x mismatch for face {i}"

        assert np.isclose(oh_cell_centroids[i*2+1], gold_zone_centroids[i, 2], atol=coord_tol), f"Centroid y mismatch for face {i}"
        assert np.isclose(oh_cell_centroids[i*2+1], dg2d_zone_centroids[i, 2], atol=coord_tol), f"Centroid y mismatch for face {i}"

def test_get_number_of_edges_inside_wall():
    with OmegaHMesh(omega_h_mesh_to_convert) as mesh:
        number_of_edges_inside_wall = mesh.get_number_of_edges_inside_wall()
        assert number_of_edges_inside_wall == 14, f"Number of edges_inside_wall (expected 14) = {number_of_edges_inside_wall}"
...
def test_get_wall_adjacent_triangles():
    ...
    assert np.all(ref_plasma_side == plasma_side), f"Plasma side cell id mismatch {ref_plasma_side=} != {plasma_side=}"
    assert np.all(ref_target_side == target_side), f"Target side cell id mismatch {ref_target_side=} != {target_side=}"
