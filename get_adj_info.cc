...
void walk_on_curve_square_curve(Omega_h::Mesh &mesh, std::vector<int> &sorted_curve_edges, 
        std::vector<int> &sorted_curve_nodes, int starting_node);
void walk_on_curve_wall_curve(Omega_h::Mesh &mesh, std::vector<int> &sorted_curve_edges, 
        std::vector<int> &sorted_curve_nodes, int starting_node);
void sort_curve(Omega_h::Mesh& mesh, std::vector<int>& sorted_curve_edges, std::vector<int>& sorted_curve_nodes, std::string type);
...
int main(int argc, char **argv) {
  ...
  sort_curve(mesh, sorted_square_curve_edges, sorted_square_curve_nodes, "square");
  std::cout << "Number of sorted square nodes: " << sorted_square_curve_nodes.size() << "\n";
  // sim numbers of the boundary nodes
  std::vector<int> sorted_square_curve_nodes_sim(sorted_square_curve_nodes.size());
  ...
  if (printflag){
    ...
    for (int i = 0; i < sorted_square_curve_nodes.size(); ++i) {
      std::cout << sorted_square_curve_nodes[i] << "\t" << sorted_square_curve_nodes_sim[i] << "\n";
    }
  ...
  if (printflag){
    ...
    for (int i = 0; i < sorted_wall_curve_nodes.size(); ++i) {
      std::cout << sorted_wall_curve_nodes[i] << "\t" << sorted_wall_curve_nodes_sim[i] << "\n";
    }
  ...
  std::cout << "Number of wall nodes - post sim numbering: " << sorted_wall_curve_nodes.size() << "\n";
  std::cout << "Number of square nodes - post sim numbering: " << sorted_square_curve_nodes.size() << "\n";
  ...
  netCDF::NcDim wallnodes_dim = ncFile.addDim("wallnodes_dim", sorted_wall_curve_nodes.size());
  netCDF::NcVar wallnodesVar =
      ncFile.addVar("wall_nodes", netCDF::ncInt, wallnodes_dim);

  // * write the wall edges to the netcdf file
  netCDF::NcDim walledges_dim = ncFile.addDim("walledges_dim", sorted_wall_curve_edges.size());
...
void sort_curve(Omega_h::Mesh& mesh, std::vector<int>& sorted_curve_edges, std::vector<int>& sorted_curve_nodes, std::string type)
{
  ...
  if (type == "square") {
    ...
    Omega_h::Read<Omega_h::I8> exposed_sides = Omega_h::mark_exposed_sides(&mesh);
  ...
  if (type == "wall") {
  std::cout << "Closest node to the vertical axis: " << starting_node << " and simNumbering is "
            << node2sim[starting_node] << "\n";
  ...
  if(type == "square"){
    walk_on_curve_square_curve(mesh, sorted_curve_edges, sorted_curve_nodes, starting_node);
  }
  else if(type == "wall"){
    walk_on_curve_wall_curve(mesh, sorted_curve_edges, sorted_curve_nodes, starting_node);
  }
}
...
void walk_on_curve_square_curve(Omega_h::Mesh &mesh, std::vector<int> &sorted_curve_edges, 
        std::vector<int> &sorted_curve_nodes, int starting_node)
        {
          ...
          for (int i = 0; i < sorted_curve_nodes.size(); ++i) {
            ...
            int nAdjacentEdges = node2edgeOffsets[current_node + 1] - node2edgeOffsets[current_node];
...
void walk_on_curve_wall_curve(Omega_h::Mesh &mesh, std::vector<int> &sorted_curve_edges, 
        std::vector<int> &sorted_curve_nodes, int starting_node)
        {
          ...
          for (int i = 0; i < sorted_curve_nodes.size(); ++i) {
            ...
            for (int j = 0; j < nAdjacentEdges; ++j) {
              if (onwall[edge2nodeNodes[2 * adjacentEdges[j]]] && onwall[edge2nodeNodes[2 * adjacentEdges[j] + 1]]) {
                wallAdjacentEdges.push_back(adjacentEdges[j]);
              }
            }
            if (wallAdjacentEdges.size() < 2)
            {
              std::cerr << "There should be 2 or more wall edges adjacent to a wall node. Has: " << wallAdjacentEdges.size()  << " Exiting...\n";
              return;
            }
            if (wallAdjacentEdges.size() > 2 && i != 0) {
              ...
              {
                closeness[j] = abs(edge_class_id[wallAdjacentEdges[j]] - current_edge_class_id);
              }
              // get the least 2 closeness
              std::vector<int> least_closeness(2);
              std::pair<int, int> closest_edge_indices = findTwoSmallestIndices(closeness);
              std::vector<int> closeness_based_wallAdjacentEdges(2);
              closeness_based_wallAdjacentEdges[0] = wallAdjacentEdges[closest_edge_indices.first];
              closeness_based_wallAdjacentEdges[1] = wallAdjacentEdges[closest_edge_indices.second];
            ...
            // for the 1st itertation, get the next node that is below the current node: 2nd coordinate is lower
            int next_node = -1;
            int next_edge = -1;

            if (i == 0) {
              if (Omega_h::get_vector<2>(coords, wallAdjacentNodes[0])[1] < Omega_h::get_vector<2>(coords, wallAdjacentNodes[1])[1]) {
...
std::pair<int, int> findTwoSmallestIndices(const std::vector<int>& vec) {
    if (vec.size() < 2) {
        throw std::invalid_argument("The vector should contain at least two elements.");
    }
